...
class RDKitUFF(Calculator):
    """
    ASE calculator wrapping RDKit's UFF force field.
    Supports energy and forces via analytical gradients.
    """
    ...
    def calculate(self, atoms=None, properties=['energy', 'forces'], system_changes=all_changes):
        ...
        # Build RDKit mol from ASE atoms using atomic numbers
        mol = Chem.RWMol()
        for atom in self.atoms:
            rdatom = rdchem.Atom(int(atom.number))
            mol.AddAtom(rdatom)

        # Add bonds based on distance (simple covalent radius heuristic)
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                ri = covalent_radii[self.atoms[i].number]
                rj = covalent_radii[self.atoms[j].number]
                if dist < 1.3 * (ri + rj):
        ...
        if ff is None:
            raise RuntimeError(
                f"RDKit UFF: Could not construct force field. "
                f"Unsupported element? Atoms: {self.atoms.get_chemical_symbols()}"
            )

        # Energy: RDKit returns kcal/mol → convert to eV
        kcal_to_eV = 0.04336411531
        energy_kcal = ff.CalcEnergy()
        self.results['energy'] = energy_kcal * kcal_to_eV

        # Forces: RDKit returns kcal/mol/Å → convert to eV/Å
        if 'forces' in properties:
            grad = np.array(ff.CalcGrad()).reshape(n, 3)  # gradient = -force
            self.results['forces'] = -grad * kcal_to_eV


def get_calculator():
    """Return a RDKit UFF ASE calculator."""
    print('success')
    return RDKitUFF()
